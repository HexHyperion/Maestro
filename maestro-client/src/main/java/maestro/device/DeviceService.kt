...
object DeviceService {
    ...
    fun startDevice(
        ...
        when (device.platform) {
            Platform.IOS -> {
                try {
                    util.LocalSimulatorUtils.bootSimulator(device.modelId, iosDeviceSet)
                    if (device.language != null && device.country != null) {
                        ...
                        util.LocalSimulatorUtils.setDeviceLanguage(device.modelId, device.language, iosDeviceSet)
                        LocaleUtils.findIOSLocale(device.language, device.country)?.let {
                            util.LocalSimulatorUtils.setDeviceLocale(device.modelId, it, iosDeviceSet)
                        }
                        util.LocalSimulatorUtils.reboot(device.modelId, iosDeviceSet)
                    }
                    util.LocalSimulatorUtils.launchSimulator(device.modelId, iosDeviceSet)
                    util.LocalSimulatorUtils.awaitLaunch(device.modelId, iosDeviceSet)
    ...
    fun listConnectedDevices(
        ...
        return listDevices(includeWeb = includeWeb, host, port, iosDeviceSet)
    ...
    fun listAvailableForLaunchDevices(includeWeb: Boolean = false): List<Device.AvailableForLaunch> {
     ...
     fun listDevices(includeWeb: Boolean, host: String? = null, port: Int? = null, iosDeviceSet: String? = null): List<Device> {
        return listAndroidDevices(host, port) +
                listIOSDevices(iosDeviceSet) +
                if (includeWeb) {
                    listWebDevices()
                } else {
                    listOf()
    ...
    private fun listIOSDevices(iosDeviceSet: String? = null): List<Device> {
        val simctlList = try {
            util.LocalSimulatorUtils.list(iosDeviceSet)
        } catch (ignored: Exception) {
            return emptyList()
        }
     ...
     */
    fun isDeviceConnected(deviceName: String, platform: Platform, iosDeviceSet: String? = null): Device.Connected? {
        return when (platform) {
            Platform.IOS -> listIOSDevices(iosDeviceSet)
                .filterIsInstance<Device.Connected>()
     ...
     */
    fun isDeviceAvailableToLaunch(deviceName: String, platform: Platform, iosDeviceSet: String? = null): Device.AvailableForLaunch? {
     ...
     */
    fun createIosDevice(deviceName: String, device: String, os: String, iosDeviceSet: String? = null): UUID {
        val command = SimctlUtils.simctlBaseArgs(iosDeviceSet) + listOf(
            "create",
            deviceName,
    ...
    fun deleteIosDevice(uuid: String, iosDeviceSet: String? = null): Boolean {
        val command = SimctlUtils.simctlBaseArgs(iosDeviceSet) + listOf(
            "delete",
            uuid
        )
    ...
    fun killIOSDevice(deviceId: String, iosDeviceSet: String? = null): Boolean {
        val command = SimctlUtils.simctlBaseArgs(iosDeviceSet) + listOf("shutdown", deviceId)
